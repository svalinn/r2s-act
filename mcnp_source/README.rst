...
This folder contains a modified version(s) of the ``source.F90`` file, and a link to the custom-compiled DAG-MCNP executables that uses this custom ``source.F90`` file (link is only applicable for user in cnergg group on Wisconsin's CAE; see section *CNERG custom executables*).
